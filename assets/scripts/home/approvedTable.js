function getApprovedTable() {
  ...
  function loadData(response) {
    ...
    var approvedItemDataContainer = response.value;
    var approvedItemData = "";
    var i = 0;
    if (approvedItemDataContainer.length) {
      $.each(approvedItemDataContainer, function (key, value) {
        //Obtain Date
        let date;
        ...
        //Insert Data
        approvedItemData += "<tr id = approvedItemRow[" + i + "]>";
        approvedItemData += "<td id = approvedItemId[" + i + "] data-id = " + value.assignmentId + ">" + value.assignmentId + "</td>";
        if (role === "ADMIN") {
          $(".employeeName").show();
          approvedItemData += "<td id = approvedItemEmployee[" + i + "]>" + value.employeeUsername + "</td>";
          $("#approvedPagination").attr("colspan", 5);
        }
        approvedItemData += "<td id = approvedItemName[" + i + "]>" + value.itemName + "</td>";
        approvedItemData += "<td id = approvedItemQuantity[" + i + "]>" + value.quantity + "</td>";
        approvedItemData += "<td id = approvedItemUpdated[" + i + "]>" + date + "</td>";
        approvedItemData += "</tr>";
        i++;
      });
